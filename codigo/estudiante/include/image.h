...
void set_pixel (int i, int j, byte value);
    ...
    /**
     * @brief Metodo que invierte una imagen plana alojada en la ruta origen y se guarda en destino
    ...
    /**
     * @brief Metodo constante que calcula la media de bytes de una subimagen y la devuelve
     * @param i FIla inicial a partir de la cual se hace la media
     * @param j Columna inicial a partir de la cual se hace la media
     * @param height Numero de filas en las que se hace la media
     * @param width Numero de columnas en las que se hace la media
     * @return Devuelve la media de bytes de esa subimagen
     */
    double Mean (int i, int j, int height, int width) const;

    
    /**
     * @brief Metodo constante que genera un icono como reducción de una imagen
     * @param factor Factor de reduccion de la imagen original con respecto al icono
     * @pre factor > 0
     * @return La imagen iconizada
     * @post La imagen no se modifica
     * @post La imagen resultando tiene tamaño (filas/factor)*(columnas/factor)
     */
    Image Subsample(int factor) const;

    /**
     * @brief Metodo constante que genera una subimagen
     * @param nrow Fila inicial para recortar
     * @param ncol Columna inicial para recortar
     * @param height  Numero de filas a cortar
     * @param width Numero de columnas a cortar
     * @return Imagen recortada
     * @post EL objeto que llama a la funcion no se corta
     */
    Image Crop(int nrow, int ncol, int height, int width) const;

    /**
     * @brief Metodo constante que calcula el zoom de una imagen
     * @return Devuelve la imagen una vez hecho el zoom
     */
    Image Zoom2X() const;
